...
func fetchFilingTool() mcp.Tool {
	return mcp.NewTool("fetch_filing",
		mcp.WithDescription("Download a specific filing document to the local cache. Use the document_url from list_filings output. "+
			"Returns {local_path, content_type, file_size_bytes, source, is_archive?, total_in_archive?, truncated?}. "+
			"When is_archive is true the filing was delivered as a zip archive; call list_zip_contents with the same ch_number and document_url to see all extracted files. "+
...
func getLatestTool() mcp.Tool {
	return mcp.NewTool("get_latest",
		mcp.WithDescription("Fetch the most recent filing of a given category for a UK company. Combines list_filings and fetch_filing in a single call. "+
			"Returns the same fields as fetch_filing: {local_path, content_type, file_size_bytes, source, is_archive?, total_in_archive?, truncated?}. "+
			"When is_archive is true the filing was delivered as a zip archive; call list_zip_contents with the same ch_number and the document_url from list_filings to see all extracted files."),
		mcp.WithString("ch_number",
			mcp.Required(),
			mcp.Description("Companies House number"),
		),
...
func listZipContentsTool() mcp.Tool {
	return mcp.NewTool("list_zip_contents",
		mcp.WithDescription("List all documents extracted from a zip archive filing. "+
			"Use this when fetch_filing or get_latest returns is_archive: true. "+
			"Returns {entries: [{filename, local_path, content_type, file_size_bytes, is_primary}], total_in_archive?, truncated?}. "+
			"The primary document (is_primary true) is the one whose local_path was returned by fetch_filing; other entries (e.g. a PDF alongside an iXBRL) can be read directly via their local_path. "+
		...
		mcp.WithString("document_url",
			mcp.Required(),
			mcp.Description("Document URL from list_filings output — the same URL used with fetch_filing"),
		),
	)
}
